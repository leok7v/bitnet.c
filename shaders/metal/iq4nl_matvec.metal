...
kernel void iq4nl_matvec(device const uchar *weights [[buffer(0)]],
    ...
    float acc = 0.0f;
    if (global_row < rows) {
        uint row_byte = global_row * blocks_per_row * 18;
        for (uint b = 0; b < blocks_per_row; b++) {
            device const uchar *block = weights + row_byte + b * 18;
            ...
            for (uint i = 0; i < BYTES_PER_THREAD; i++) {
                ...
                uchar q = qs[byte_i];
                int lo = kvalues_iq4nl[q & 0xFu];
                int hi = kvalues_iq4nl[q >> 4];
                acc += d * float(lo) * x[x_base + elem_base + byte_i];
                acc += d * float(hi) * x[x_base + elem_base + byte_i + 16];
            }
        }
    }
